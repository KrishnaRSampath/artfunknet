...
var getArtistID = function(artist_db, artist_name) {
	...
	if (artist_db.find({"artistName": artist_name}).count() == 0) {
		...
		var status = artist_db.findOne({"artistName": artist_name}).status;
		...
		switch (status) {
			case "pending":
				...
				return "";
			case "denied":
				alert("This artist has not been accepted");
				return "";
			case "approved": return artist_db.findOne({"artistName": artist_name})._id;
			default: return "";
		}
	}
}
